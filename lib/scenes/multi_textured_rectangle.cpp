#include "com/visualization/scenes/multi_textured_rectangle.h"
...
namespace com
{
  ...
  {
    const char *multi_textured_rectangle_vertex_source =
    ...
    const char *multi_textured_rectangle_fragment_source =
      "varying vec3 Color;\n"
      "varying vec2 Texcoord;\n"
      ...
      "void main()\n"
      "{\n"
      "  gl_FragColor = mix(texture2D(tex_kitty, Texcoord), "
      "    texture2D(tex_puppy, Texcoord), 0.5);\n"
      "}";

    // Shader sources
    multi_textured_rectangle::
      multi_textured_rectangle() :
      width_(0),
      height_(0),
      retina_scale_(1.0),
    ...
    multi_textured_rectangle::~multi_textured_rectangle()
    {}

    bool multi_textured_rectangle::initialize(
      QOpenGLFunctions &gl_functions)
    {
      if (!initialized_.load()) {
        ...
        shader_program_.add_vertex_shader_source(
          multi_textured_rectangle_vertex_source);
        shader_program_.add_fragment_shader_source(
          multi_textured_rectangle_fragment_source);
        ...
        boost::filesystem::path image_path1 = application::exe_dir() /
          "sample2.png";
        width = 0;
        height = 0;
        logging::info(log_tag, "Loading image from:", image_path1);
        image = SOIL_load_image(image_path1.string().c_str(),
          &width, &height, 0, SOIL_LOAD_RGB);
        gl_functions.glActiveTexture(GL_TEXTURE1);
        GL_CALL(gl_functions, "glActiveTexture", log_tag);
        gl_functions.glGenTextures(1, &tex1_);
        GL_CALL(gl_functions, "glGenTextures", log_tag);
        gl_functions.glBindTexture(GL_TEXTURE_2D, tex1_);
        GL_CALL(gl_functions, "glBindTexture", log_tag);
        gl_functions.glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0,
          GL_RGB, GL_UNSIGNED_BYTE, image);
        GL_CALL(gl_functions, "glTexImage2D", log_tag);
    ...
    void multi_textured_rectangle::resize(int parent_width,
    ...
    void multi_textured_rectangle::pre_draw(
      QOpenGLFunctions &gl_functions)
    {
      std::string log_tag = logging::log_tag_for_this(*this);
      ...
      gl_functions.glBindTexture(GL_TEXTURE_2D, tex1_);
      GL_CALL(gl_functions, "glBindTexture", log_tag);
      GLuint puppy_uniform = gl_functions.glGetUniformLocation(
        shader_program_.id(), "tex_puppy");
      GL_CALL(gl_functions, "glGetUniformLocation", log_tag);
      gl_functions.glUniform1i(puppy_uniform, 1);
      GL_CALL(gl_functions, "glUniform1i", log_tag);
      gl_functions.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
        GL_CLAMP_TO_EDGE);
      GL_CALL(gl_functions, "glTexParameteri", log_tag);
    ...
    void multi_textured_rectangle::draw(
      QOpenGLFunctions &gl_functions)
    {
      std::string log_tag = logging::log_tag_for_this(*this);
    ...
    void multi_textured_rectangle::post_draw(
      QOpenGLFunctions &gl_functions)
    {
      std::string log_tag = logging::log_tag_for_this(*this);
    ...
    bool multi_textured_rectangle::de_initialize(
      QOpenGLFunctions &gl_functions)
    {
      std::string log_tag = logging::log_tag_for_this(*this);
      ...
      gl_functions.glDeleteTextures(1, &tex1_);
